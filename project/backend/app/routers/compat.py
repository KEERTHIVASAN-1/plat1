...
from backend.app.models import RunRequest, SubmitRequest, SubmissionOut, UserOut, UserLogin, UserCreate, TokenResponse
from backend.app.routers.auth import get_current_user, login as auth_login, register as auth_register
...
from backend.app.routers.problems import list_problems as api_list_problems, get_problem as api_get_problem
...
async def submit_legacy(req: SubmitRequest, current_user: UserOut = Depends(get_current_user)):
...
async def login_legacy(input: UserLogin):
    return await auth_login(input)

@router.post("/auth/register", response_model=TokenResponse)
async def register_legacy(input: UserCreate):
    return await auth_register(input)
...
async def submissions_legacy(userId: str, current_user: UserOut = Depends(get_current_user)):
    return await api_user_submissions(userId, current_user)

@router.get("/admin/participants")
async def admin_participants_legacy(admin: UserOut = Depends(get_current_user)):
    return await api_admin_participants(admin)

@router.get("/admin/participant/{participant_id}")
async def admin_participant_legacy(participant_id: str, admin: UserOut = Depends(get_current_user)):
    return await api_admin_participant(participant_id, admin)

@router.patch("/admin/participant/{participant_id}/eligibility")
async def admin_toggle_eligibility_legacy(participant_id: str, admin: UserOut = Depends(get_current_user)):
    return await api_admin_toggle_eligibility(participant_id, admin)

@router.patch("/admin/participant/{participant_id}/password")
async def admin_set_password_legacy(participant_id: str, password: str = None, admin: UserOut = Depends(get_current_user)):
    return await api_admin_set_password(participant_id, password, admin)

@router.post("/timer/start")
async def timer_start_legacy(roundId: str = None, durationSeconds: int = None, body: dict = Body(None)):
    b = body or {}
...
async def timer_resume_legacy(roundId: str = None, body: dict = Body(None)):
    ...
    if not rid:
        raise HTTPException(422, "roundId required")
    # resume behaves like restart (sets status active and resets startTime)
    return await api_timer_restart(rid)

@router.post("/timer/schedule")
async def timer_schedule_legacy(roundId: str = None, startAt: str = None, durationSeconds: int = None, body: dict = Body(None)):
    b = body or {}
    rid = roundId or b.get("roundId")
    ...
    if not rid:
        raise HTTPException(422, "roundId required")
    return await api_timer_configure(rid, duration=duration, scheduledStart=start_at)
